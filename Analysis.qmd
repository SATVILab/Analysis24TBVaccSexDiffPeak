...
png("Q-Q Plot of Residuals for Uninfected Individuals.png")
qqnorm(beta_uninfected$residuals)
...
# Q-Q plot for the residuals of the beta regression model
png("Q-Q Plot of Residuals for Infected Individuals.png")
qqnorm(beta_infected$residuals)
...
```{r}
mod_list <- list()
for (infxn in c("uninfected", "infected")) {
  data_mod_inf <- data_tidy_vacc_summed |>
      dplyr::filter(infxn == infxn)
  for (vacc in unique(data_mod_inf$vaccine)) {
    mod_list[[infxn]][[vacc]] <- fit_reg_beta(
      data_mod_inf,
      ref_vacc = "h56"
    )
  }
...
full_model_inf <- betareg(response_proportion ~ vaccine*sex, data_tidy_vacc_summed |> 
    dplyr::filter(infxn =="infected") |>
    dplyr::mutate(vaccine = factor(.data$vaccine, levels = c("h56", "h1", "mva85a", "m72"))))

full_model_inf_mva85a <- betareg(response_proportion ~ vaccine*sex, data_tidy_vacc_summed |> 
    dplyr::filter(infxn =="infected") |>
    dplyr::mutate(vaccine = factor(.data$vaccine, levels = c("mva85a", "h56", "h1", "m72"))))

full_model_inf_h1 <- betareg(response_proportion ~ vaccine*sex, data_tidy_vacc_summed |> 
    dplyr::filter(infxn =="infected") |>
    dplyr::mutate(vaccine = factor(.data$vaccine, levels = c("h1", "mva85a", "h56", "m72"))))
full_model_inf_m72 <- betareg(response_proportion ~ vaccine*sex, data_tidy_vacc_summed |> 
    dplyr::filter(infxn =="infected") |>
    dplyr::mutate(vaccine = factor(.data$vaccine, levels = c("m72", "h1", "mva85a", "h56"))))

#for table for presentation slides:estimates of effects sizes and their CI
# Extract coefficients and confidence intervals
coef_summary <- summary(full_model_inf)
coef_estimates <- coef_summary$coefficients$mu[, "Estimate"]

conf_intervals <- confint(full_model_inf)
conf_intervals_mva85a <- confint(full_model_inf_mva85a)
conf_intervals_h1 <- confint(full_model_inf_h1)
conf_intervals_m72 <- confint(full_model_inf_m72)

# Create a data frame for plotting
plot_data <- data.frame(
  Vaccine = c("H56", "H1", "MVA85A", "M72"),
  Estimate = c(coef_estimates["sexmale"],  # For H56
    coef_estimates["sexmale"] + coef_estimates["vaccineh1:sexmale"],  # For H1
    coef_estimates["sexmale"] + coef_estimates["vaccinemva85a:sexmale"],  # For MVA85A,
    coef_estimates["sexmale"] + coef_estimates["vaccinem72:sexmale"]  # For M72
  ),
  CI_Lower = c(conf_intervals["sexmale", "2.5 %"],  # For H56
    conf_intervals_h1["sexmale", "2.5 %"],  # For H1
    conf_intervals_mva85a["sexmale", "2.5 %"],  # For MVA85A,
    conf_intervals_m72["sexmale", "2.5 %"]  # For M72
  ),
  CI_Upper = c(
    conf_intervals["sexmale", "97.5 %"],  # For H56
    conf_intervals_h1["sexmale", "97.5 %"],  # For H1
    conf_intervals_mva85a["sexmale", "97.5 %"],  # For MVA85A
    conf_intervals_m72["sexmale", "97.5 %"]  # For M72
  ),

  Color = c("H56", "H1", "MVA85A", "M72")              
)

# Plot the effect sizes with confidence intervals
plot_infected2<-ggplot(plot_data, aes(x = Vaccine, y = Estimate, ymin = CI_Lower, ymax = CI_Upper,width=0.1,color=Color)) +
  geom_pointrange() +
  labs(
    x = 'Vaccine',
    y = "Estimate",
  ...
  theme_cowplot_bg()+
  scale_color_manual(values = c("H56" = "red", 
                                "H1" = "blue", 
                                "MVA85A" = "purple",
                                "M72" = "green"))+  
  background_grid(major='y') +
  theme(
    ...
    axis.text.x = element_text(size = 16),  # Adjust size for x-axis text labels
    axis.text.y = element_text(size = 16),   # Adjust size for y-axis text labels
    legend.position = "none",
    plot.caption = element_text(size = 16)  # Adjust the caption size
  )                            

ggsave('Plot of estimates of effect sizes and CI.png',plot_infected2,width = 10, height = 6)
...
# Wald test
Wald_inf <- car::linearHypothesis(full_model_inf, c("sexmale = 0", "vaccineh1:sexmale = 0","vaccinemva85a:sexmale = 0", "vaccinem72:sexmale = 0"))
...
```{r}
full_model_uninf <- betareg(response_proportion ~ vaccine*sex, data = data_tidy_vacc_summed|>
                 dplyr::filter(infxn == "uninfected")|>
    dplyr::mutate(vaccine = factor(.data$vaccine, levels = c("h56", "h1", "mva85a","bcg", "m72"))))
​
full_model_uninf_mva85a <- betareg(response_proportion ~ vaccine*sex, data_tidy_vacc_summed |> 
    dplyr::filter(infxn =="infected") |>
    dplyr::mutate(vaccine = factor(.data$vaccine, levels = c("mva85a", "bcg", "h56","h1", "m72"))))

full_model_uninf_h1 <- betareg(response_proportion ~ vaccine*sex, data_tidy_vacc_summed |> 
    dplyr::filter(infxn =="infected") |>
    dplyr::mutate(vaccine = factor(.data$vaccine, levels = c("h1", "mva85a", "bcg","h56", "m72"))))

full_model_uninf_bcg <- betareg(response_proportion ~ vaccine*sex, data_tidy_vacc_summed |> 
    dplyr::filter(infxn =="infected") |>
    dplyr::mutate(vaccine = factor(.data$vaccine, levels = c("bcg","h56","h1", "mva85a", "m72"))))

full_model_uninf_m72 <- betareg(response_proportion ~ vaccine*sex, data_tidy_vacc_summed |> 
    dplyr::filter(infxn =="infected") |>
    dplyr::mutate(vaccine = factor(.data$vaccine, levels = c("m72", "bcg","h56","h1", "mva85a"))))

#for table for presentation slides:estimates of effects sizes and their CI
# Extract coefficients and confidence intervals
coef_summary <- summary(full_model_uninf)
coef_estimates <- coef_summary$coefficients$mu[, "Estimate"]

conf_intervals <- confint(full_model_uninf)
conf_intervals_mva85a <- confint(full_model_uninf_mva85a)
conf_intervals_h1 <- confint(full_model_uninf_h1)
conf_intervals_bcg <- confint(full_model_uninf_bcg)
conf_intervals_m72 <- confint(full_model_uninf_m72)

# Create a data frame for plotting
plot_data <- data.frame(
  Vaccine = c("H56", "H1", "MVA85A","BCG", "M72"),
  Estimate = c(
    coef_estimates["sexmale"],  # For H56
    coef_estimates["sexmale"] + coef_estimates["vaccineh1:sexmale"],  # For H1
    coef_estimates["sexmale"] + coef_estimates["vaccinemva85a:sexmale"],  # For MVA85A
    coef_estimates["sexmale"] + coef_estimates["vaccinebcg:sexmale"],  # For bcg
    coef_estimates["sexmale"] + coef_estimates["vaccinem72:sexmale"]  # For M72
  ),
  CI_Lower = c(
    conf_intervals["sexmale", "2.5 %"],  # For H56
    conf_intervals_h1["sexmale", "2.5 %"],  # For H1
    conf_intervals_mva85a["sexmale", "2.5 %"],  # For MVA85A
    conf_intervals_bcg["sexmale", "2.5 %"],  # for bcg
    conf_intervals_m72["sexmale", "2.5 %"]  # For M72
  ),
  CI_Upper = c(
    conf_intervals["sexmale", "97.5 %"],  # For H56
    conf_intervals_h1["sexmale", "97.5 %"],  # For H1
    conf_intervals_mva85a["sexmale", "97.5 %"],  # For MVA85A
    conf_intervals_bcg["sexmale", "97.5 %"],  # For bcg
    conf_intervals_m72["sexmale", "97.5 %"]  # For M72
  ),
  Color = c("H56", "H1", "MVA85A", "BCG", "M72")              
)

# Plot the effect sizes with confidence intervals
plot_infected3 <- ggplot(plot_data, aes(x = Vaccine, y = Estimate, ymin = CI_Lower, ymax = CI_Upper,width=0.1,color=Color)) +
  geom_pointrange() +
  labs(
    x = 'Vaccine',
    y = "Estimate",
...
ggsave('Plot of estimates of effect sizes and CI.png',plot_infected3,width = 10, height = 6)
